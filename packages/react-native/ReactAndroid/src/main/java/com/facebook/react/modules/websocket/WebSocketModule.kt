...
public class WebSocketModule(context: ReactApplicationContext?) :
  ...
  private val cookieHandler: ForwardingCookieHandler = ForwardingCookieHandler(context)
  ...
  override fun connect(
    ...
    if (protocols != null && protocols.size() > 0) {
      ...
      for (i in 0..<protocols.size()) {
        val v: String = protocols.getString(i).trim()
    ...
    private fun getDefaultOrigin(uri: String): String {
      try {
        ...
        when (requestURI.scheme) {
          ...
          "ws" -> scheme += "http"
          "http",
          "https" -> scheme += requestURI.scheme
          else -> {}
        }
